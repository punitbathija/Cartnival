...
const createOrder = async (data, lineItems) => {
  ...
  console.log(itemDetails.map((item) => console.log(item)));
  ...
  const newOrder = new Order({
    ...
    orderItems: [
      {
        name: itemDetails.map((item) => item.description),
        photo: image,
        quantity: itemDetails.map((item) => item.quantity),
        price: itemDetails.map((item) => item.amount_subtotal),
        product: itemId,
      },
    ],
    paymentInfo: {
